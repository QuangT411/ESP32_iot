...
// ===================================================================
//  SETUP — thứ tự: GPIO → Cảm biến → SD → WiFi → Online services
// ===================================================================
void setup()
{
  Serial.begin(115200);
  ...
  if (millis() - lastSend >= SEND_WINDOW_MS) {
    if (sampleCount > 0) {
      ...
      if (!offlineMode && Firebase.ready()) {
        sendToFirebase(avgT, avgH, avgP, avgLux, avgSoil, dt, nowEpoch);

        Serial.println("--- ✅ 5m AVG → Firebase ---");
      } else {
        // --- OFFLINE: lưu vào SD Card ---
        saveToSD(avgT, avgH, avgP, avgLux, avgSoil,
                 flowRateLMin, totalVolumeL, dt);

        Serial.println("--- 💾 5m AVG → SD Card (offline) ---");
      }

      Serial.printf("T=%.2fC H=%.2f%% P=%.2fhPa Lux=%.2f Soil=%.2f%% Flow=%.3f Vol=%.3f\n",
                    avgT, avgH, avgP, avgLux, avgSoil, flowRateLMin, totalVolumeL);
